...
| [acceleration_final_2026-07-03.md](./acceleration_final_2026-07-03.md) | **⭐ 加速最终定稿（2026-07-03，自包含）** —— 先读这篇：最终配方（持久化跨轮 trainer + worker 评估 + ALF replicas/manifest 缓存 + run 级服务 + 热终评 + 仅最终 HF 导出 + WS fused）、总账（**70 轮：ALFWorld ≈41→16.7 h ×2.5、WebShop ≈32→9.4 h ×3.5**；对 wiring 栈 −43%/−54%）、每个瓶颈为何倒下（冷启动 → 环境管道 → 交接 → GPU）、以及一步步的完整历程含全部死胡同（dyn-bsz +8~11%、lanes/WS-replicas 在 paper 规模打平、one_step_off 实得 −10% 且离策略）。中文版：[acceleration_final_2026-07-03_cn.md](./acceleration_final_2026-07-03_cn.md) |
...
| [nanorollout_comparison_cn.md](./nanorollout_comparison_cn.md) | **对比 NanoRollout（2026-07-01）** —— 与 UCSD/cocoa-org 的 rollout-as-a-service 框架的代码级对比：边界粒度（step 级 env-as-a-service vs episode 级 `/run`）、token 保真度（token-native vs miles TITO proxy + TIS）、可靠性契约、加速效果的可比与不可比、各自优劣、互相可借鉴清单，以及**独立文本环境 Agent RL framework 的双层 API 设计提案**（§7）。 |
