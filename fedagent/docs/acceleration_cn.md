...
- **每个 (client, round) 一个全新的 OS 子进程** —— 干净隔离，保证 GPU 释放。
  一轮内的 client 是**串行的**（`for c in selected`，`run_fed.py:1046`）。
- **轮次是硬屏障：** 第 `r+1` 轮在 `FedAvg(r)`+`merge(r)` 产出 `model_r` 之前无法启动。
- **Eval 是独立子进程**（`eval_global`，`run_fed.py:717`，`trainer.val_only=true`）→ 它
  要付出*第二次*完整的冷启动，并且它在该轮**之后**运行，不阻塞任何东西但被它阻塞。
- **Env 服务**（WebShop/ALFWorld）是**每个 client 的远程 FastAPI 服务**，按轮**惰性**
  启动（仅该轮被选中的 client），在聚合前拆除（`run_fed.py:1386`）。
...
`run_fed.inject_rollout_mode`（`run_fed.py:647`）把 windowed manager 注入到 train 与 eval
...
**Windowed 默认值崩溃 —— 已确认为 release blocker。** `rollout_mode=windowed`（新默认值）+ 原生 `agent.yaml`（只注册了 `gym_text`）→ `AttributeError: 'GymTextAgentLoop' object has no attribute 'run_episode_windowed'`（[windowed_manager.py](../agent_loops/windowed_manager.py)#L152）。windowed 此前只在*显式*的 `tools/verl08_migration/poc/windowed` agent 配置下才是绿的；任何基于原生 `agent.yaml` 的配置都会崩溃。→ **暂停的 windowed 配置迁移**（注册 `gym_text_windowed`）是 windowed-as-default 的前置条件。
...
- **可行性：** `select_clients(r+1, …)` 是确定性的（seed 为 `base_seed + round - 1`，
  `run_fed.py:799-811`）→ 下一轮的 client 在第 `r` 轮结束前就已知。端口按 client 索引
...
- **最干净的接缝：** 把 `RayPPOTrainer.fit()` 的主体抽成一个可重入的 `_fit_one_client()`，
 ...
 （`init_workers()`）是一次性的；从 `self.global_steps = 0`（L1383）往下已经是 per-`fit()`，
  会变成 per-client。overlay 驱动的接缝是 **`run_fed.py:1046`**（`for c in selected`
...
（`run_fed.py:877`），*按 client 索引，不依赖顺序*）。
...
GPU / `/dev/shm` 冲突 —— 隔离只靠各自的 `CUDA_VISIBLE_DEVICES` + `RAY_TMPDIR`。（有一个*不显眼*的共享 `/tmp`
例外 —— FSDP→vLLM 权重传输 socket —— 在 2 个 job 时**靠运气没撞**，直到第 3 个并发 job 加入才死锁；见下面的
**第二个鲁棒性 bug**，也是为什么 #3 自身的共存只有在那个修复*之后*才安全。）时序：
...
**发现并修掉了第二个鲁棒性 bug（FSDP→vLLM 权重传输 socket —— 同一家族）。** 推到**第三个**并发 verl job
（2 train + 1 eval，§7.7）暴露出一个更深的冲突。verl 的 FSDP→vLLM 权重同步（`bucketed_weight_transfer.py`）
把 rollout 引擎的新权重经由一个**共享 `/tmp` 上的 ZMQ IPC socket** 送出：`ipc:///tmp/rl-colocate-zmq-<job_id>-replica-<r>-rank-<lr>.sock`，
*正是*以 **Ray job id** 命名空间化来保持并发 job 互不相交。但 FedAgent 把每个 client/eval 跑成它**自己隔离的
Ray 集群**（`RAY_TMPDIR`），而每个新集群分配的**第一个 job id 都是同一个 `01000000`**（已验证 —— 两个集群，id
相同）→ 所有 job 算出**同一个** socket 路径 → 各 sender 的 `os.remove` + 重新 `bind` 竞态把它们交叉接线，权重
`send` **死锁**（GPU 确认：两个 trainer 在 0% util 下**挂了 44 分钟**，`do_epoll_wait` 卡在 `update_weights` 里；
env 服务健康但**零 session** —— rollout 从未开始）。2-client 的情形此前是**靠运气没撞**；3 个 job 必死锁 ——
所以这威胁的是 **#3 本身**，而不只是 eval-parallel。**修复（overlay 优先）：** `run_fed` 给每个启动的 verl 子进程
导出一个**唯一的 `VERL_RAY_JOB_ID`**（每进程 tag + role/client/round），再加一个**2 行 verl patch** 让 sender
（`vllm_rollout.py`）和 receiver（`vllm_async_server.py`）**遵从这个覆盖**而非那个相撞的 job id；原生单集群 run
（未设覆盖）逐字节不变。已 GPU 验证：之前死锁的那个 3-job 布局现在三个全部闭合（`rc=0`；watcher 看到权重同步通过）。
**与 FedAvg 端口 bug 同样的教训 —— verl 的 per-job 隔离假设了 ONE 个共享 Ray 集群；FedAgent 的 cluster-per-client
模型打破了这个假设** —— 两个修复都给共享主机资源（rendezvous 端口 / `/tmp` socket）一个 per-job 唯一的名字。
...
| #3 parallel clients | 无（FedAvg 与顺序无关、seed 按 client 索引） | ✅ 安全；单节点 2×2 已 GPU 验证（1.5B −35%）+ 修掉**两个**并发 bug：FedAvg rendezvous 端口（`--standalone`）与 FSDP→vLLM 权重传输 socket（`VERL_RAY_JOB_ID`）—— §Lever #3 / §7.7 |
...
### 7.7 "2 个 client 各占 1 GPU 训练 + 2 GPU eval" 布局 —— 正确性 OK，但**不是**快路径
**问题（一个硬件分配的想法）：** 在 4-GPU 节点上，**2 个 client 各占 1 GPU（并行）+ eval 在空出的 2 GPU 上**
—— 即 **#3 ⊗ #1** —— 是否比保持每 client 2 GPU 更快？它把 eval *藏*到专用卡上，这是 #3-用满-4-GPU 做不到的。
已在 1.5B、paper 设置、n=64 WebShop val 上 GPU 测试。

**它证明了什么（全部 GPU 确认）：**
- **3-job 4-GPU 共存可行** —— 2 个 trainer（各 1 GPU）+ 1 个 eval（2 GPU）加载在不相交的卡上（A/B 峰值 26 GB、
  eval 40 GB），无 OOM —— **但只有在**权重传输 socket 修复之后（§Lever #3，*第二个* bug；正是这个布局暴露了
  3-job 死锁）。
- **Eval 确实被藏起来：** 独立的 val pass（忠实 —— 驱动 `run_fed._build_eval` → 与循环的 eval 所用同一条 verl
  val-only 代码路径）在 **407 s** 完成，而训练跑了 **995 s**，所以 eval 占用空出的卡 <½ 个 round，额外 wall-clock
  成本为**零**。

**但它不是快路径** —— 1-GPU 训练占主导：

| 布局（4-GPU 节点，M=2，per-round） | 墙钟 |
|---|---|
| 4-GPU 单 client（无 client 并行） | 558 s |
| **#3 —— 2 client × 2 GPU 并发**（仅训练） | **727 s** |
| #3 + **`eval_mode=worker`**（热引擎 eval，无 eval 冷启动） | **≈ 845 s** |
| #3 + 串行（冷启动）eval | 727 + 407 ≈ 1134 s |
| **本布局 —— 2 client × 1 GPU + 2-GPU eval（隐藏）** | **995 s** |

1-GPU 训练实测 **~180–226 s/optimizer-step**，该 round **995 s** —— 对比 2 GPU 的 **725 s**（**1.37× 慢，不是 2×**：
WebShop rollout 是 **env-latency-bound** 的，所以把 GPU 减半几乎不动 rollout；只有 FSDP fwd/bwd + vLLM generation
缩放，而那是 wall-clock 中的少数）。藏起 eval 省了 ~407 s，但 1-GPU 的代价（995 − 727 = **+268 s/round**）是**每个
round** 都付，而正确的基线不是*串行* eval —— 是 **`eval_mode=worker`**，它已经让 eval 近乎免费（热引擎、无第二次
冷启动）。所以 **#3（2 GPU/client）+ worker eval（≈ 845 s）打赢本布局的 995 s**：别为了腾出 eval 卡就把训练
饿到 1 GPU。**裁定：正确性 OK，且干净地演示了 eval 能藏在专用 GPU 上 —— 但 #3 + worker eval 才是推荐的单节点快
...
下面的一切都是**仅 overlay** —— 但有**一个有意的例外**：一个**对 `others/verl` 的 2 行 patch**
（`vllm_rollout.py` + `vllm_async_server.py`），让 FSDP→vLLM 权重传输 socket 遵从一个 `VERL_RAY_JOB_ID`
覆盖（§7.7 / §Lever #3）。verl 本体保持**上游 pristine**；这 2 行被抓成
...
（在环境搭建时 apply，base commit `7aed6b2`）。那是一个真正的**上游假设修复**（verl 的 per-job 隔离假设了一个共享
Ray 集群），值得提 PR；其余一切仍然不 fork。
