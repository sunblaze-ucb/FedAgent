...
| [acceleration_final_2026-07-03.md](./acceleration_final_2026-07-03.md) | **⭐ THE DEFINITIVE ACCELERATION RECORD (2026-07-03, self-contained)** — read this one first: the final recipe (persistent cross-round trainer + worker eval + ALF replicas/manifest cache + run-scoped services + hot final eval + final-only HF export + WS fused), the accounting (**70-round: ALFWorld ≈41→16.7 h ×2.5, WebShop ≈32→9.4 h ×3.5**; vs the wiring stack −43 %/−54 %), why each bottleneck fell (cold starts → env plumbing → handoffs → GPU), and the full station-by-station journey incl. every dead end (dyn-bsz +8–11 %, lanes/WS-replicas wash at paper scale, one_step_off −10 % realized & off-policy). |
...
| [nanorollout_comparison_cn.md](./nanorollout_comparison_cn.md) | **Comparison vs NanoRollout (2026-07-01, Chinese)** — code-level comparison against the UCSD/cocoa-org rollout-as-a-service framework: boundary granularity (step-level env-as-a-service vs episode-level `/run`), token fidelity (token-native vs miles' TITO proxy + TIS), reliability contracts, what is/isn't comparable in the speedup numbers, mutual borrow-list, and a **two-layer-API design proposal for a standalone text-env agent-RL framework** (§7). |
| [acceleration_frontier_2026-07-02.md](./acceleration_frontier_2026-07-02.md) | **Frontier study (2026-07-02)** — can FedAgent still be accelerated (yes: ~800–1000 s of addressable plumbing, phase-decomposed), is more async useful (saturated within the on-policy bar; one-step-off exists but is off-policy), what verl 0.8 still offers (audit + two probed refutations: dyn-bsz slower, fused-kernels = WebShop garnish). |
| [acceleration_tier2_2026-07-02.md](./acceleration_tier2_2026-07-02.md) | **Tier-2 optional knobs (2026-07-02)** — every remaining lever as a default-OFF config knob (manifest cache, `service_scope: run`, hot final eval, `hf_export: final`, lanes, `one_step_off`), each equivalence-gated (all at/below the measured 9.3e-5 same-config noise floor); optimal combos measured on the REAL paper configs: ALFWorld steady round −32 %, WebShop −56 %; 70-round projections **29.3 → 16.7 h / 20.4 → 9.4 h**; lanes = wash on both envs; the sys.path shadowing fix. |
