...
class WebAgentTextEnv(gym.Env):
    ...
    def __init__(
        ...
        print(f"[DEBUG] WebAgentTextEnv: self.kwargs={self.kwargs}")
        print(f"[DEBUG] WebAgentTextEnv: self.kwargs.get('shuffle_seed', None)={self.kwargs.get('shuffle_seed', None)}")
        self.server = SimServer(
            ...
            self.kwargs.get('shuffle_seed', None),  # add shuffle_seed parameter, default value is 42
            self.kwargs.get('catalog_filter_asins', None),  # env-level heterogeneity
            self.kwargs.get('bm25_in_memory_config', None),  # transition-level (BM25 Reweighting)
            self.kwargs.get('extra_products', None),  # transition-level (Lookalike Injection)
            self.kwargs.get('search_engine_variant', None),  # search-engine TYPE swap
...
class SimServer:
    ...
    def __init__(
        ...
        catalog_filter_asins (`list[str]` or None) -- For env-level heterogeneity:
            if provided, search/click only see products whose ASIN is in this list,
            but goals are still generated from the full 1000-product pool so that
            self.goals length is the same across all federated clients (see
        ...
        # Track 1: full products (catalog_filter_asins=None) -> used for goal generation
        # so that self.goals is identical across all federated clients.
        # Lookalike Injection: extra_products (adversarial lookalike) is appended through
        # load_products so they go through identical processing as base catalog.
        # When set, search backend must rebuild the BM25 index over the augmented
        # corpus — auto-route to InMemoryBM25Searcher below.
        self.full_products, self.full_product_item_dict, self.product_prices, _ = \
            load_products(filepath=file_path, attrpath=attr_path,
                          num_products=num_products, human_goals=human_goals,
                          catalog_filter_asins=None,
                          extra_products=extra_products)
        # Track 2: filtered products -> used for search/click; this is what changes per client.
        if catalog_filter_asins is not None:
            catalog_set = set(catalog_filter_asins)
            self.all_products = [p for p in self.full_products if p['asin'] in catalog_set]
            self.product_item_dict = {p['asin']: p for p in self.all_products}
            print(f'[SimServer] catalog filter active: '
                  f'{len(self.all_products)}/{len(self.full_products)} products '
                  f'(goals still generated from full 1000-pool)')
        else:
            self.all_products = self.full_products
            self.product_item_dict = self.full_product_item_dict
        # Lucene index is built once over the full 1000 products (file
        # search_engine/indexes_1k); BM25 hits are post-filtered by
        # `if asin in product_item_dict` in get_top_n_product_from_keywords (engine.py:172),
        # so per-client catalog filtering takes effect without rebuilding the index.
        # If bm25_in_memory_config is set (kwarg, or env var fallback for probe orchestration),
        # swap in InMemoryBM25Searcher with per-client (fields, k1, b) — see
        # docs/heterogeneity.md
        if bm25_in_memory_config is None:
            _env_cfg = os.environ.get('WEBSHOP_BM25_VARIANT_JSON', '').strip()
            if _env_cfg:
                bm25_in_memory_config = json.loads(_env_cfg)
        # Lookalike Injection: when extra_products are present, the default Lucene index
        # doesn't know about lookalike ASINs → BM25 won't surface them. Force
        # InMemoryBM25Searcher with default settings so lookalikes are indexed.
        if extra_products and bm25_in_memory_config is None:
            bm25_in_memory_config = {
                'fields': ['name', 'Title', 'description', 'features', 'BulletPoints'],
                'k1': 1.2, 'b': 0.75,
            }
            print(f'[SimServer] auto-routing to in-memory BM25 because '
                  f'extra_products={len(extra_products)} (lookalike injection)')
        if bm25_in_memory_config is not None:
            print(f'[SimServer] in-memory BM25 variant active: '
                  f'fields={bm25_in_memory_config.get("fields")} '
                  f'k1={bm25_in_memory_config.get("k1", 1.2)} '
                  f'b={bm25_in_memory_config.get("b", 0.75)}')
        if search_engine_variant is not None:
            print(f'[SimServer] search-engine variant active: {search_engine_variant}')
        self.search_engine = init_search_engine(
            num_products=num_products,
            in_memory_config=bm25_in_memory_config,
            products=self.full_products,
            search_engine_variant=search_engine_variant,
        )
        # IMPORTANT: goals must be generated from full_products so that self.goals
        # has the same length across all federated clients. Otherwise different
        # clients would index goals[500] -> different goal, breaking goal_idxs.
        self.goals = get_goals(self.full_products, self.product_prices, human_goals)
        ...
        if shuffle_seed is None:
            ...
            print(f"[DEBUG] SimServer: shuffle_seed was None, set to default 42")
