...
| script | purpose |
|---|---|
| [`gen_paper_configs.py`](gen_paper_configs.py) | regenerates the 194-cell paper matrix under `fedagent/config/paper/` (`--accel` → the accelerated twins in `paper_accelerated/`; `--accel --n-gpus 1` → the single-H100 twins in `paper_accelerated_1gpu/`) |
...
| [`verify_alfworld_kernel_variants.py`](env_heterogeneity/verify_alfworld_kernel_variants.py) | planner-level proof for the ALFWorld kernel-variant arms (`obs_variant` / `dyn_variant` / `goal_variant`) | for a task-type-stratified game sample × every pool variant: the rewritten kernel is still fast-downward-solvable through a real `PddlEnv`, `obs_variant` leaves the default plan winning verbatim, `dyn`/`goal` variants change it ([dev_doc](../fedagent/docs/dev_doc/alfworld_env_heterogeneity.md), verification layer 2) |
| [`gen_alfworld_cells.py`](env_heterogeneity/gen_alfworld_cells.py) | the ALFWorld (query × env) cell library | substrate for the τ-strictly-zero control arm ([dev_doc](../fedagent/docs/dev_doc/alfworld_query_env_decoupling.md)): every train query transplanted into compatible host trials, indexed and validated; the runtime arm is not wired yet |
| [`rollout_reference_occupancy.py`](env_heterogeneity/rollout_reference_occupancy.py) → [`measure_delta_occ.py`](env_heterogeneity/measure_delta_occ.py) → [`report_delta_occ.py`](env_heterogeneity/report_delta_occ.py) | the occupancy-weighted divergence δ_eff of every WebShop env-het variant (paper Appendix L) | stage 1 rolls the reference policy over the 64-goal val set and records its (s,a) stream; stage 2 replays each (s,a) in every client's backend and reports the pairwise disagreement rate (the plug-in TV estimate); stage 3 renders the tables |
