...
echo "[ALF16BLK3 start $(date)] host=$(hostname)"
...
timeout 9000 python -m fedagent.fed.run_fed --config "$G/alf_ppo_pool16_block.yaml" \
  ...
  --output-dir "$RUNS/alf_ppo_pool16_block" > "$G/valf_ppo_pool16_block_v3.log" 2>&1
echo "[ALF16BLK3 exit=$? $(date)]"
grep -iE "FEDERATED LOOP CLOSED|client [01] round 1 OK|FedAvg|warmed [0-9]+ envs|ReadTimeout|out of memory|Killed|FAILED" "$G/valf_ppo_pool16_block_v3.log" | grep -vE "atexit|dump_compile" | tail -10
