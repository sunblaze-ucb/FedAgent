...
echo "[ALFR4096 start $(date)] host=$(hostname)"
...
rm -rf "$RUNS/alf_ppo_resp4096" 2>/dev/null
timeout 9000 python -m fedagent.fed.run_fed --config "$G/alf_ppo_resp4096.yaml" --rounds 1 --clients 2 \
  ...
  --output-dir "$RUNS/alf_ppo_resp4096" > "$G/valf_ppo_resp4096.log" 2>&1
echo "[ALFR4096 exit=$? $(date)]"
grep -iE "FEDERATED LOOP CLOSED|global_step:[0-9]|response_length/clip_ratio|ReadTimeout|out of memory" "$G/valf_ppo_resp4096.log" | tail -6
