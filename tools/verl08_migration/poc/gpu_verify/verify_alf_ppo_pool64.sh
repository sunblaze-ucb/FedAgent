...
echo "[ALFPPO64 start $(date)] host=$(hostname)"
...
timeout 9000 python -m fedagent.fed.run_fed --config "$G/alf_ppo_pool64.yaml" \
  ...
  --output-dir "$RUNS/alf_ppo_pool64" > "$G/valf_ppo_pool64.log" 2>&1
echo "[ALFPPO64 exit=$? $(date)]"
grep -iE "FEDERATED LOOP CLOSED|client [01] round 1 OK|FedAvg|warmed [0-9]+ envs|ReadTimeout|out of memory|Killed|FAILED" "$G/valf_ppo_pool64.log" | grep -vE "atexit|dump_compile" | tail -10
