...
WEBSHOP_PORT="$PORT" WEBSHOP_POOL_SIZE=2 \
  bash "$REPO_ROOT/fedagent/envs/webshop/service/run_service.sh" > /tmp/ws_svc_test.log 2>&1 &
SVC=$!
cleanup() { kill "$SVC" 2>/dev/null; pkill -f "fedagent.envs.webshop.service.server" 2>/dev/null; }
trap cleanup EXIT
