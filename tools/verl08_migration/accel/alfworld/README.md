...
| `alf_t2_{base,cache,scope,feval,all}.yaml` | **Tier-2 knob A/B arms** (2c × 2r × 1 step at paper caps, 48-game val): same-config replicate + manifest cache + `service_scope: run` + `final_eval_mode: worker` + all-four (`../run_t2_stack.sh`) | replicate = noise floor **9.293e-5**; walls **−18/−16/−11/−24%**, every arm ≤ floor, acceleration.md §10.1 |
